 ...
 <table class="table">
    ...
    <thead class="table-active">
      ...
      <i class="fa-regular fa-location-pen"></i>
            ...
            <td><img src="{{asset('storage/'.$posts->image_path)}}" width="100" height="100" alt="image"/></td>

            <td>{{$posts->nick}}</td>
            <td>{{$posts->email}}</td>
            <td>{{$posts->tytul}}</td>
            <td>{{$posts->created_at}}</td>
            <td> <a href="#" class="btn btn-primary  btn-sm enabled" role="button" aria-disabled="true" title="view">

            <i class="fa-solid fa fa-eye"></i></a>

                        <a href="{{route('edit',$posts->id)}}" class="btn btn-success btn-sm enabled" title="edit">
